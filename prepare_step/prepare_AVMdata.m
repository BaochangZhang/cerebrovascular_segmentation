function [ImageT,Iout,TempVessel] = prepare_AVMdata(Image_path,brainMask_path,Empirical_value)
% This function is used to prepare the data with AVM, which will implement the
% following steps: 
...
% [ImageT,Iout,TempVessel] = prepare_AVMdata(Image_path,brainMask_path,Empirical_value)
% Input:
%      Image_path: the path of original image(datatype: nii or nii.gz)
%      brainMask_path: the path of image without skull
%      Empirical_value:a hard threshold to segment the
%      vessel,i.e.Empirical_value = 880
% Output:
...
%   load data(original image and brain mask)    
    ...
    Image_edge=zeros(size(Image));
    for i = 1:size(Image,3)
        Image_edge(:,:,i)=edge(Mask(:,:,i),'sobel');
    end
    ImageT(ImageT==0)=prctile(Image(Image_edge>0),99)*1.2;
